...
def get_args():
    ...
    parser.add_argument("--save-dir", default="model_3")
...
def evaluate(model, dataloader, device, args, true_triples=None, valid_triples=None):
    ...
    with tqdm(dataloader, desc="testing") as pbar:
        for samples in pbar:
            ...
            out, logits = model.logits(tmp_source, tmp_prefix, cts=False)
            ...
            for l in range(2, max_len):
                ...
                out, all_logits = model.logits(source.view(bb, -1), prefix.view(bb, -1), cts=False)
...
def train(args):
    ...
    # evaluate(models_new, test_loader, device, args, train_valid, eval_valid)
    if args.iter:
        ...
        if args.iter:
            if curr_iter_epoch == 0: # start next iteration
                ...
                if curr_iter > 0:
                    ...
                    train_set.data_aug_index = data_aug(train_set.tgt_lines)
                    train_loader = DataLoader(train_set, batch_size=args.batch_size, collate_fn=train_set.collate_fn, shuffle=True)
                # new scheduler
